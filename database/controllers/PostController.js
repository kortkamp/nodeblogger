...
function getModelFields() {
  ...
  for (const field of postFieldsArray) {
    postDataFields.push({
      name: field,
...
async function addView(id) {
  try {
    ...
    return await Post.update({ views: post.views + 1 }, { where: { id } });
  } catch (err) {
    console.log(err);
  }
}
async function addLike(id) {
  try {
    const post = await getPostById(id);
    return await Post.update({ likes: post.likes + 1 }, { where: { id } });
  } catch (err) {
    console.log(err);
  }
}
...
module.exports = {
  getPostById, listAllPosts, postArticle, getPostContent, listAllHeaders, getModelFields, deleteArticle, addView, addLike,
};
